...
- Floating components are positioned by a **prop**, not by reverse-engineering
  their CSS.

  `<GraphToolbar>`, `<GraphSimControls>`, `<GraphSelectionActions>` and
  `<GraphMinimap>` gain:

  ```tsx
  position?: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right'
  offset?: number // distance from both anchored edges, default 12
  ```

  Defaults match today's placement, so nothing moves unless you ask it to. This
  finishes a convention `<GraphInspector dock>` already had; the floating chrome
  just never got it.

  ## Why

  Repositioning the toolbar used to require this:

  ```tsx
  <GraphToolbar style={{ left: "auto", top: "auto", right: 8, bottom: 8 }} />
  ```

  `style` merged _over_ a default `left: 12`, so `{ right: 8 }` alone left both
  insets live, `left` won in LTR, and the component simply did not move — with
  the caller's value sitting in the DOM doing nothing. You had to know the
  default's CSS to override it. Now:

  ```tsx
  <GraphToolbar position="bottom-right" offset={8} />
  ```

  ## Also fixed

  `style` inset overrides now RESOLVE against the defaults instead of layering
  under them, per axis: `{ right: 8 }` drops the default `left`, while a lone
  `{ top: 40 }` nudges vertically and keeps the horizontal anchor. So the escape
  hatch works too, not just the prop.

  `className` continues to drop the default styling entirely — now including
  placement, consistently across all four (`GraphSimControls` and `GraphMinimap`
  previously ignored it).

  `GraphCorner` and `DEFAULT_CORNER_OFFSET` are exported for hosts that want to
  type their own placement state.

- `<GraphTooltip>` now serves edge hovers, and `instance.getEdge(id)` resolves an
  edge id to its record.

  Core tracked `store.hover.edgeId` and `SelectionState.edgeIds`, and engine edge
  picking already worked — but nothing could turn an edge id back into an edge, so no
...
- v0.9.1 remediation: six verified correctness fixes.
...
- `<Graph>` forwards a `services` construction option.

  A custom revision-aware service set — most usefully an async `expansion`
  service backed by the host's own data source — could previously be injected
  only through the low-level `createGraphInstance({ services })` seam, which
  forced any consumer with server-backed expansion off `<Graph>` entirely. The
  prop is construction-only (D7, same contract as `engine`/`searchIndex`/
  `fitViewOnFirstData`): read once at mount, a change requires a keyed remount.
  With it, `expandNode`, the context menu's Expand item, and the pending-
  expansion affordances all run against the host's service.
